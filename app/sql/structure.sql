...
-- TABLE : role
-- Rôles utilisateurs (client, employé, admin)
...
CREATE TABLE `commande` (
  ...
  `statut` enum('en_attente','validee','en_preparation','en_cours_livraison','livree','attente_retour_materiel','terminee','annulee') COLLATE utf8mb4_unicode_ci DEFAULT 'en_attente',
...
-- TABLE : avis
-- Avis clients sur les commandes
...
CREATE TABLE `prete` (
  `commande_numero` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL,
  ...
  CONSTRAINT `prete_ibfk_1` FOREIGN KEY (`commande_numero`) REFERENCES `commande` (`numero_commande`) ON DELETE CASCADE,
...
CREATE TABLE `utilise_promotion` (
  ...
  CONSTRAINT `utilise_promotion_ibfk_1` FOREIGN KEY (`commande_numero`) REFERENCES `commande` (`numero_commande`) ON DELETE CASCADE,
