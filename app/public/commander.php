...
function updateSummary() {
    ...
    // Frais de livraison (on ne peut pas calculer exactement sans la ville)
    const fraisLivraisonEstime = '<em>calculés selon la ville</em>';
    
    const prixTotal = prixMenu; // + frais livraison
    ...
    summaryDiv.innerHTML = `
        <table class="table table-sm">
            ...
            <tr>
                ...
                <td class="text-end">${(prixParPersonne * nombrePersonnes).toFixed(2).replace('.', ',')} €</td>
            </tr>
            ${reduction > 0 ? `
            <tr class="text-success">
                <td>Réduction -10%:</td>
                <td class="text-end">-${reduction.toFixed(2).replace('.', ',')} €</td>
            </tr>
            ` : ''}
            <tr>
                <td>Prix menu final:</td>
                <td class="text-end"><strong>${prixMenu.toFixed(2).replace('.', ',')} €</strong></td>
            </tr>
            <tr>
